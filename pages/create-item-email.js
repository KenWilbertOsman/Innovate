...
export default function CreateItem() {
    ...
    async function createItem() {
        ...
        //hash the username for user tracking
        let hash = await sha1(username)
        console.log(hash)
        theHash = hash
        if (!username || !userphone || !recname || !recaddress || !recphone || !fragile || !fileUrl || !mass) return
    ...
    async function createSale(url) {
        ...
        formRef.current.transaction_id.value = theHash
        console.log(formRef.current.transaction_id.value)

        emailjs.sendForm('gmail', 'template_q33bjv8', formRef.current, '0vW7VSGuG-AMsmAKM')
        .then((result) => {
    ...
    //to indicate if user click the done button after looking at the transaction id, the popup to be gone
    //have to change to sending email    
    async function userDone(){
        setPopup(false)
        router.push('/create-item-email')
    }
    ...
    if (popup && loadingState == "loaded") return (
        ...
        <div className = "flex flex-col items-center rounded-lg shadow-xl h-4/6">
            ...
            <h3 className="text-center text-theme-blue font-bold text-lg break-normal m-5 break-all"> {hashed}</h3>
            <button 
    ...
    return (
        ...
        <div className="flex justify-center">
            <form ref = {formRef} className="w-half lg:w-1/2  flex flex-col pb-12" onSubmit={callFunction}>
                <input
                    placeholder="Name"
                ...
                <input
                    ...
                    className="mt-2 border rounded p-4"
                    // pattern="[0-9].{9,}" required //from 0 to 9 with 9 or more characters
                ...
                />
                <textarea
                    //Parcel will be delivered to {{rec_name}} at {{rec_address}}
                ...
                <label for="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                    ...
                    <p className="mb-3 mx-3 lg:mx-0 text-sm text-gray-500 dark:text-gray-400"><span className="font-semibold">Click to Upload Parcel Image</span> or Drag and Drop</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">PNG or JPG</p>
                    </div>
                <input id="dropzone-file" type="file" className="hidden" name = "Asset" onChange = {onChange}/>
                </label>
                </div> 
                {
                    fileUrl && (
