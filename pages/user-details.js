...
export default function Detail() {
    ...
    async function loadNFTDetails() {
        ...
        const index = data['h']
        const nftDetail = await marketContract.fetchAllNFT()
        const items = await Promise.all(nftDetail.map(async i => {
            ...
            if (item.hash == index) { return item }
        }))

        let newItems = await Promise.all(items.filter(i => (i != undefined)))
        ...
        newItems[0]['addressName'] = accountsFetch
        setNfts(newItems)
                                    ...
                                    <div className='leading-loose my-2 text-xs lg:text-base'>
                                        <div className='flex flex-row'>
                                            ...
                                            <div className='basis-1/2 '>
                                                <span className='float-right break-all'>{nft.name}
                                                </span>
                                            </div>
                                        </div>
                                        <div className='flex flex-row'>
